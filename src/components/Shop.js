...
const Shop = ({ isMobile, lookup }) => (
  <div class="p16 fv">
    ...
    {isMobile ? (
      <div class="tac pv8">{brands.map(c1)}</div>
    ) : (
      <div class={`fw pb16 ${isMobile ? '' : 'ph8'}`}>
        {brands.map(b => (
          <div class="f w20 p8">{c1(b)}</div>
        ))}
      </div>
    )}
    <hr />
    <h2>本月特价 On Sale</h2>
    {isMobile ? (
      <div class="tac pv8">{imgs.map(n => c2(lookup, n))}</div>
    ) : (
      <div class={`fw pb16 ${isMobile ? '' : 'ph8'}`}>
        {imgs.map(n => (
          <div class="f w20 p8">{c2(lookup, n)}</div>
        ))}
      </div>
    )}
    <hr />
...
const c1 = b => (
  <div>
    <img src={'/images/brands/' + b + '.jpg'} />
  </div>
)
const c2 = (l, n) => (
  <div>
    <img class="w100" src={cdurl(l, 'slider-5', n)} />
  </div>
)
