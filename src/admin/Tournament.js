...
const b1 = p =>
  <Fragment>
    ...
    {p.canDisplaySchedule ? <Link class="item" to={`/admin/schedules/${p.t.id}`}>Schedules</Link> : null}
...
export default compose(
  ...
  withProps(({ standing, t }) => ({
    ...
    canGenerateSchedule: !isEmpty(t.isSingle ? t.players : t.teams) && isEmpty(t.schedules),
    canGenerateSchedule2: !t.has2half && !isEmpty(t.teams) && isNil(t.teams[0].group) && !isEmpty(t.schedules) && !isEmpty(standing) && standing.length > 2,
  ...
  withProps(p => ({
    numOfButtons: [p.canDisplaySchedule, p.canGenerateSchedule, p.canGenerateSchedule2, p.canGenerateGroup, p.canGenerateKO].filter(x => x === true).length + 1
  })),
