...
const { findById, split2, getNameById, use, sortBy } = require('.')
...
e.initdata = async d =>
  e.initdocs(d || (await httpGet(`${process.env.GITHUB_DB}db.json`)))

e.backup = () =>
  Promise.all(allDocs.map(e.get)).then(l =>
...
e.addToList = (doc, id, list, obj) =>
  db.collection(doc).update({ id: +id }, { $addToSet: { [list]: obj } })

e.replaceList = (doc, id, list, obj) =>
  db
    .collection(doc)
    .update(
...
e.getPlayerRating = (id, date) =>
  use(
    ...
    .then(x =>
      x.length === 0 ? e.getById('players', id) : sortBy('rating', x)[0]
    )
    .then(x => (x || {}).rating)
...
e.changePlayer = (tid, p1, p2) =>
  e.getById('tournaments', tid).then(t =>
    e.get('players').then(ps => {
      if (
        t.isSingle &&
        ps.find(x => x.id == p1) &&
        ps.find(x => x.id == p2) &&
...
e.groupmatch = (id, grp, body) =>
  e.getById('tournaments', id).then(t => {
    if (t.schedules) {
      ...
      return e
        .clearList('tournaments', +id, 'games')
        .then(r =>
          serial(newGames.concat(games), g =>
            e.addToList('tournaments', +id, 'games', g)
          )
        )
    } else {
      return 'N/A'
