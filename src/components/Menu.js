...
const items = (ms, s) =>
  ms.map((x, i) => (
    <Link to={'/4' + (i === 0 ? '' : x)} onClick={() => s(false)}>
      <Menu.Item name={x} />
    </Link>
  ))
const _menu = c => (
  <Menu inverted color="blue" style={{ margin: 0 }}>
    {c}
  </Menu>
)
...
// const popup = (t, img, f) => <Popup content={t} position="bottom center" trigger={<a className="cp" onClick={f}><img src={`images/${img}.png`} className="mr4" alt="" /></a>} />
